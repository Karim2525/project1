...
DataModule2.ADOQposlygi.SQL.Text:=  ' SELECT poslygi.[Код реклами], poslygi.[Тип послуги], poslygi.[Ціна за місяць], Kompanii.Назва'
+' FROM Kompanii INNER JOIN poslygi ON Kompanii.[Код компанії] = poslygi.[Код компаніі];';

 DataModule2.ADOQposlygi.Open;
//'SELECT poslygi.[Код реклами], poslygi.[Тип послуги], poslygi.[Ціна за місяць], Kompanii.Назва'
//+' FROM poslygi, Kompanii WHERE (poslygi.[Код реклами]=Digivir.[Код реклами]) AND (Digivir.[Код компаніі]=Kompanii.[Код компаніі])';

// INNER JOIN (Kompanii INNER JOIN Digivir ON Kompanii.[Код компанії] = Digivir.[Код компаніі]) ON poslygi.[Код реклами] = Digivir.[Код реклами]';
...
DataModule2.ADOTKompanii.Locate('Назва',DBGrid1.Fields[3].AsString,[loCaseInsensitive, loPartialKey]);  // !!!ОБРАТИ Внимание!!! Для того что бы добавить в таблицу "Меню" код категории
...
DataModule2.ADOQWork.SQL.Text:= 'DELETE * FROM poslygi Where (poslygi.[Код реклами] = '+DBGrid1.Fields[0].AsString+')';
...
begin
DataModule2.ADOQposlygi.Close;
DataModule2.ADOQposlygi.SQL.Text:='INSERT INTO poslygi ( [Тип послуги], [Ціна за місяць], [Код компаніі] ) values ( "'+Edit3.Text+'", '+Edit5.Text+', '+IntToStr(DBLookupComboBox1.KeyValue)+')';
DataModule2.ADOQposlygi.ExecSQL;

//DataModule2.ADOQposlygi.Close;
//DataModule2.ADOQposlygi.SQL.Text:= 'SELECT poslygi.[Код реклами], poslygi.[Тип послуги], poslygi.[Ціна за місяць], Kompanii.Назва '
//+' FROM poslygi INNER JOIN (Kompanii INNER JOIN Digivir ON Kompanii.[Код компанії] = Digivir.[Код компаніі]) ON poslygi.[Код реклами] = Digivir.[Код реклами];';
//DataModule2.ADOQposlygi.Open;
...
DataModule2.ADOQDigivir.SQL.Text:='SELECT * FROM Digivir  WHERE Digivir.[Дата]= :dt';
...
DataModule2.ADOQposlygi.Close;
DataModule2.ADOQposlygi.SQL.Text:= ' SELECT poslygi.[Код реклами], poslygi.[Тип послуги], poslygi.[Ціна за місяць], Kompanii.Назва'
+' FROM Kompanii INNER JOIN poslygi ON Kompanii.[Код компанії] = poslygi.[Код компаніі] where poslygi.[Тип послуги] like "%'+Edit1.Text+'%" ORDER BY poslygi.[Код реклами]';
DataModule2.ADOQposlygi.Open;
end;
...
DataModule2.ADOQWork.Close;
DataModule2.ADOQWork.SQL.Text:='UPDATE poslygi SET poslygi.[Тип послуги] = "'+Edit3.Text+'", poslygi.[Код компаніі]= "'+IntToStr(DBLookupComboBox1.KeyValue)+'",   poslygi.[Ціна за місяць]='+Edit5.Text+' '
+' WHERE (((poslygi.[Код реклами])='+DBGrid1.Fields[0].AsString+'))';
DataModule2.ADOQWork.ExecSQL;
...
begin
DataModule2.ADOQDigivir.Close;
DataModule2.ADOQDigivir.SQL.Text:='SELECT Digivir.[Код договіру], Digivir.[Код реклами], Digivir.[Код працівника], Digivir.[Кількість місяців], Digivir.[Код компаніі], Digivir.[Ціна], Digivir.[Код продажі], Digivir.[Дата] FROM[Digivir]';
DataModule2.ADOQDigivir.Open;
end;
...
DataModule2.ADOQposlygi.Close;
DataModule2.ADOQposlygi.SQL.Text:= ' SELECT poslygi.[Код реклами], poslygi.[Тип послуги], poslygi.[Ціна за місяць], Kompanii.Назва'
+' FROM Kompanii INNER JOIN poslygi ON Kompanii.[Код компанії] = poslygi.[Код компаніі] where poslygi.[Код реклами] = '+Edit2.Text+' ORDER BY poslygi.[Код реклами]';
DataModule2.ADOQposlygi.Open;
end;
